...
def add_world_edges(graph, world_edge_radius=0.03, edge_stats_path="edge_stats.json"):
    ...
    edge_index = radius_search(world_pos, world_pos, radius=world_edge_radius, return_dists=False, return_points=False)
    ...
    candidate_edges = set(
        (int(src), int(dst)) for src, dst in zip(edge_index[0].tolist(), edge_index[1].tolist())
    )
    world_edges = torch.tensor(
        [list(edge) for edge in candidate_edges if edge not in mesh_edges]
    ,dtype=torch.int32, device=graph.device).T  # shape: (2, num_world_edges)

    if world_edges.shape[1] == 0:
        # No new world edges to add
        return graph, graph.edata["x"], torch.zeros((0, graph.edata["x"].shape[1]), device=graph.device)
