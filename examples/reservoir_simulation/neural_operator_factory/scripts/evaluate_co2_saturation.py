...
"""
CO2-specific evaluation script for saturation prediction.

This script uses CO2 dataset conventions (permeability mask, grid spacing,
time labels) and is not intended for other datasets.
"""
...
def main():
    ...
    if model_type == "ufno":
        ...
        model = UFNONet(
            ...
            decoder_activation_fn=model_config.get("decoder_activation_fn", None),
        ).to(device)
